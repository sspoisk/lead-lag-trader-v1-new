...
def simulate(follower: np.ndarray,
    ...
    for imp in impulses:
        ...
        if confirm_candles > 0 and confirm_pct > 0:
            # Смотрим на свечу входа (entry_idx) и несколько предыдущих
            look_start = max(0, entry_idx - confirm_candles)
            alt_open  = follower[look_start, 1]   # open первой смотровой свечи
            alt_close = follower[entry_idx, 4]     # close свечи входа (или open)
            if alt_open <= 0:
                continue
...
def main():
    ...
    print(f"\n  Вывод:")
    # Найти лучший по avg_pct (исключая нулевые)
    scenarios = [r for r in [r_a, r_b, r_c, r_d, r_e] if r and r.get('n', 0) >= 5]
    if scenarios:
        best = max(scenarios, key=lambda x: x['avg_pct'])
        worst = r_a  # baseline
        if best['avg_pct'] > worst['avg_pct']:
            improvement = best['avg_pct'] - worst['avg_pct']
            label_map2 = {
            ...
            print(f"  Лучший: {label_map2.get(best['label'])} "
                  f"(avg: {best['avg_pct']:+.3f}% vs baseline {worst['avg_pct']:+.3f}%, "
                  f"улучшение: {improvement:+.3f}%/сделку)")
            if best['avg_pct'] < 0:
                print(f"  ⚠ Все варианты убыточны — стратегия требует фундаментального пересмотра")
            else:
                print(f"  ✓ Фильтр даёт прибыльные сделки — стоит реализовывать")
    print(f"{'='*65}\n")
